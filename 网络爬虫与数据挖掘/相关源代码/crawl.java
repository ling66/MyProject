 ...
 * example:
 * 1. crawl -politeness 0 -maxpages 600 http://www.theage.com.au/digital-life/mobiles
 * 2. crawl -maxpages 20 http://www.theage.com.au/digital-life/mobiles
 * 3. crawl -politeness 2 http://www.theage.com.au/digital-life/mobiles
 * 4. crawl http://www.theage.com.au/digital-life/mobiles
 */
...
//	/*
	public static void main(String[] args) {
		...
		// User input format:
		// crawl -politeness 0 -maxpages 600 http://www.theage.com.au/digital-life/mobiles
		for (String s : args) {
			inputArrayList.add(s);
		}
		
		crawl tCral = new crawl();
//		tCral.maxpages = 600;
//		tCral.delayTime = 0;
//		tCral.inputURL = "http://www.theage.com.au/digital-life/mobiles";
		//get the input information
		for(int i=0;i<inputArrayList.size();i++)
		{
			if(inputArrayList.get(i).contains("-politeness"))
				tCral.delayTime = Integer.parseInt(inputArrayList.get(i+1));
			else if(inputArrayList.get(i).contains("-maxpages"))
				tCral.maxpages = Integer.parseInt(inputArrayList.get(i+1));
			else if(inputArrayList.get(i).contains("http:"))
				tCral.inputURL = inputArrayList.get(i);
		}

		System.out.println("User Input Information:");
		System.out.println("Politeness: "+tCral.delayTime);
		System.out.println("Maxpages: "+tCral.maxpages);
		System.out.println("URL: "+tCral.inputURL);
		System.out.println("----------------------------Crawler Start------------------------------------");
//		if(1==1)
//			return;
		//check input link type
		if (!tCral.checkPageContentType(tCral.inputURL)) {
		...
		//write result file
		//format: document number,address eg. 1,http://www.rmit.edu.au/login.html
		StringBuffer currentLineSB = new StringBuffer();
		for(int i=0;i<tCral.relDownloadedPagesArrayList.size();i++) 
			currentLineSB.append("1").append(",").append(tCral.urlDocNumHahsMap.get(tCral.relDownloadedPagesArrayList.get(i)))
				.append(",").append(tCral.relDownloadedPagesArrayList.get(i)).append("\n");
		for(int i=0;i<tCral.irrelDownloadedPagesArrayList.size();i++)
			currentLineSB.append("0").append(",").append(tCral.urlDocNumHahsMap.get(tCral.irrelDownloadedPagesArrayList.get(i)))
			.append(",").append(tCral.irrelDownloadedPagesArrayList.get(i)).append("\n");
		FileOperation.writeFile("./basic/pages", currentLineSB.toString(), false);
...
//	*/
	...
	public void visitPageRecursion(String tURL) {
		//stop condition : if find enough pages or this page has been visited before
		...
		//if urlArrayList is null, that means some error happened in getPageLinks() method
...
//			try to add the relative link to the front of collection
//			urlClass = this.classifyPage(curURL);
//			if(urlClass.equals("1"))
//				this.unVisitLinkedList.addFirst(curURL);
...
//				this.unVisitLinkedList.add(curURL);
		...
		System.out.println("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^");
		System.out.println("----------------------------------------------------------------------------------------");
		System.out.println("Current Download Relevant files:");
		for(int i=0;i<this.relDownloadedPagesArrayList.size();i++) 
			System.out.println((i+1)+"-"+this.relDownloadedPagesArrayList.get(i)+"-"
					+this.urlDocNumHahsMap.get(this.relDownloadedPagesArrayList.get(i)));
		System.out.println("----------------------------------------------------------------");
		System.out.println("Current Download Irrelevant files:");
		for(int i=0;i<this.irrelDownloadedPagesArrayList.size();i++) 
			System.out.println((i+1)+"-"+this.irrelDownloadedPagesArrayList.get(i)+"-"
					+this.urlDocNumHahsMap.get(this.irrelDownloadedPagesArrayList.get(i)));
		System.out.println("----------------------------------------------------------------------------------------");
	...
	public ArrayList<String> getPageLinks(String tURL) {
        try {
			...
			for (Iterator<Element> it = links.iterator(); it.hasNext();) {
				...
				if(this.urlRegularExpressionChecker("1","/",name.attr("href")))
        	...
        	System.out.println("Exciption in crawl-getPageLinks!!!");
            e.printStackTrace();
            return null;
        }
	}
	/**
	 * 1. check whether the file type is text/html
	 * 2. ignore: 
	 * 0.start with the same host 
	 * 1.# eg.#nav
	 *     http://www.smh.com.au/entertainment/restaurants-and-bars/a-suburban-kitchen-for-tony-bilson-20120424-1xi4n.html#comments 
	 * 2.javascript: 
	 * 3.https eg:  https://membercentre.fairfax.com.au/NewsletterSubscription.aspx
	 * 4.relative path: eg. /lifestyle/life/blog/citykat
	 */
	public boolean checkURL(String tURL) {
		//check url type: only add url start with http: and / 
...
//			System.out.println("content-type: " + ct);
	...
	public boolean checkWhetherDownloadPage(String tURL) {
		// 1. check the number of downloadPages
		if (this.maxpages <= this.downloadPages)
			return false;
		// 2. check whether download before
		if (this.relDownloadedPagesArrayList.contains(tURL)
				|| this.irrelDownloadedPagesArrayList.contains(tURL))
			return false;

		// 3. check the number of downloaded rel and irrel pages
		// check maxpages is even or odd
		int relNum = 0;
		if ((this.maxpages % 2) == 0)
			relNum = this.maxpages / 2;
		else
			relNum = (this.maxpages + 1) / 2;

		String urlClass = this.classifyPage(tURL);
		if (urlClass.equals("1")
				&& this.relDownloadedPagesArrayList.size() >= relNum)
			return false;
		if (urlClass.equals("0")
				&& (this.irrelDownloadedPagesArrayList.size() >= (this.maxpages - relNum)))
			return false;
	...
	public void downloadPage(String pageContent,String tURL) {
		try {
			...
			// relevant page
			String filePath = "";
			if (this.classifyPage(tURL).equals("1")) {
				filePath = "./basic/rel/" + Integer.toString(this.downloadPages);
				FileOperation.writeFile(filePath, pageContent, false);
				this.relDownloadedPagesArrayList.add(tURL);
			}
			// irrelevant page
			else if (this.classifyPage(tURL).equals("0")) {
				filePath = "./basic/irrel/" + Integer.toString(this.downloadPages);
				FileOperation.writeFile(filePath, pageContent, false);
				this.irrelDownloadedPagesArrayList.add(tURL);
			}
			// store url and document number into hashmap
			this.urlDocNumHahsMap.put(tURL,Integer.toString(this.downloadPages));
		} catch (Exception e) {
			System.out.println("Problem downloadPage URL: "+tURL);
			e.printStackTrace();
		}
	}
	/**
	 * classify page using URL
	 */
	public String classifyPage(String tURL) {
		String domainUrl = "";
		if (this.inputURL.contains(".html") || this.inputURL.contains(".htm")
		|| this.inputURL.contains(".php") || this.inputURL.contains(".asp")|| this.inputURL.contains(".aspx")) {
			domainUrl = this.inputURL.substring(0, this.inputURL.lastIndexOf("/"));
		}
		else
			domainUrl = this.inputURL;
		
		if(tURL.contains(domainUrl))
			return "1";
		else
			return "0";
	}
	 ...
	 */
	public boolean urlRegularExpressionChecker(String type,String rule,String tURL) {
		String regEx = "";
		//type-1: prefix check
		if(type.equals("1"))
			regEx = "^"+rule+".*";
	 ...
	 * type: 1-whole page content; 2-only the text of page
	 */
	public String getPageContent(String tURL,String type) {
        try {
        	...
        	System.out.println("Exception in crawl-getPageContent!!!");
            e.printStackTrace();
            return null;
        }
	}
